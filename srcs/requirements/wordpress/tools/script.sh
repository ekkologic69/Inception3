...
wp config create --dbname=$DB_NAME --dbuser=$DB_USR --dbpass=$DB_PASS --dbhost='mariadb' --allow-root --skip-check
wp core install --url=$DOMAIN_NAME --title=$WP_TITLE --admin_user=$WP_ADMIN_USER --admin_password=$WP_ADMIN_PASS --admin_email=$WP_ADMIN_EMAIL --path=/var/www/html/ --skip-email --allow-root
